...
if (isset($_POST['checkoutBtn'])) {
  while ($row = mysqli_fetch_array($res)) {
    ...
    $total = $row['total'];
    $sql = "select * from user where Id='$marketId'";
    $resTotal = $con->query($sql);
    $test = mysqli_fetch_array($resTotal);
    $newTotal = $test['balance'] + $total;
    $sqlstm5 = "UPDATE user SET balance = $newTotal WHERE user.Id = $marketId";
    $con->query($sqlstm5);
  }
  $sqlstm6 = "DELETE from cart WHERE user_id = $USERID";
  $con->query($sqlstm6);
...
if (isset($_POST['removeItem'])) {
  ...
  $sqlstm10 = "DELETE from cart WHERE product_id = $product_id";
  $con->query($sqlstm10);
  header("Refresh:0");
}
    ...
    <div class="body-container">
      ...
      if ($row_count == 0) {
        ...
        echo "<div class='totals'>
                ...
                <input type='submit' name='checkoutBtn' value='Checkout' class='checkout'>
                </form>
              </div>";
      }
      ?>
